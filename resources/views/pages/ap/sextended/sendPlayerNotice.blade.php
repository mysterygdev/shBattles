                          ...
                          @else
                            <form class="form-inline" method="post" action="search.php">
                              <div class="form-group mx-sm-3 mb-2">
                                <input type="text" name="searchChar" id="searchChar" placeholder="Character Name" class="form-control text-center" oninput=fetchSearchData(this.value)>
                            ...
                            <select id="dataViewer" style="display: none;">
                              @if (count($data['notice']->viewData()) > 0)
                                @foreach ($data['notice']->viewData() as $res)
                                  <option>{{$res->CharName}}</option>
                                @endforeach
                              @endif
                            </select>
                          @endif
