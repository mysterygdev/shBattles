...
class Panels
{
    ...
    {
        $query = db::table(table('logAccess') . ' as [l]')
                        ->select(['[l].UserID', '[l].UserIP', '[l].Action', '[l].ActionTime', '[w].DisplayName'])
                        ->join(table('webPresence') . ' as  [w]', '[l].UserID', '=', '[w].UserID')
                        ->limit($limit)
    ...
    public function newUsers($limit = 200)
    {
        $query = db::table(table('shCharData') . ' as C')
                        ->selectRaw('MAX([UM].[UserUID]) AS UserUID, MAX([UM].[UserID]) AS UserID, MAX([UMG].[Country]) AS Faction, MAX([UM].[Point]) AS Point, MAX([UM].[JoinDate]) AS JoinDate, MAX([ULS].[LogOutTime]) AS LogOutTime, MAX([UM].[Status]) AS Status')
                        ->join(table('shUserData') . ' as  UM', 'C.UserUID', '=', 'UM.UserUID')
                        ->join(table('shUmg') . ' as  UMG', 'C.UserUID', '=', 'UMG.UserUID')
