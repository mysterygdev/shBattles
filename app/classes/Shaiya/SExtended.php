...
class SExtended
{
    ...
    public function __construct()
    {
        ...
        $this->faction = isset($_POST['faction']) ? $this->data->purify(trim($_POST['faction'])) : false;
    }
    ...
    public function sendPlayerPM($char, $message)
    {
        if (count($this->getChar($char)) > 0) {
            if (is_numeric($char)) {
                $char = $this->getChar($char)[0]->CharName;
            }
            $command = 'pm;' . $char . ';' . '[ADMIN]' . ';' . $message . '';
            $this->sendCommand($command);
            return true;
        }
        return false;
    ...
    public function sendTradeChat($message, $faction)
    {
        $command = 'tradechat;' . '[ADMIN]' . ';' . $faction . ';' . $message . '';
        $this->sendCommand($command);
    }
}
