...
@else
  <div class="quickMenu">
    <div class="username-title text-center">
      <p>Welcome, <strong>{{$data['user']->DisplayName}}</strong></p>
      <span id="points"><strong>{{$data['user']->Point}}</strong> {{SERVER['name']}} Points</span>
    </div>
    ...
    <ul class="text-center">
      ...
      <li>
        <form id="logout" method="post" action="/auth/logout">
          <a href="#" onclick="document.getElementById('logout').submit();">{{__('logout')}}</a>
        </form>
      </li>
    </ul>
  </div>
  {{-- <div class="lk-form block-l">
        <div class="lk-title">
          <span class="username">Welcome, {{$data['user']->DisplayName}}</span>
        </div>
        <div class="lk-coins">
          <a href="" class="gold-a">deposit</a>
          <span class="coins" id="my_credits">{{$data['user']->Point}} points</span>
          <span class="username">{{SERVER['name']}} Points</span>
        </div>
        <ul>
          @adm
            <li><a href="/admin" target="_blank">Admin Panel</a></li>
